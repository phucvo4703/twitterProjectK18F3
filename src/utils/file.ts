...
//hàm xử lí file mà client đã đưa lên
...
export const handleUploadVideo = async (req: Request) => {
  const form = formidable({
    uploadDir: path.resolve(UPLOAD_VIDEO_DIR), //video k đi qua bước đệm sharp
    maxFiles: 1, //tối đa bao nhiêu
    maxFileSize: 50 * 1024 * 1024, //tối đa bao nhiêu byte
    //xài option filter để kiểm tra file có phải là image không
    filter: function ({ name, originalFilename, mimetype }) {
      //name: name|key truyền vào của <input name = bla bla>
      //originalFilename: tên file gốc
  ...
  return new Promise<File[]>((resolve, reject) => {
    form.parse(req, (err, fields, files) => {
      ...
      videos.forEach((video) => {
        ...
        //dùng đến mới + đuôi của tên cũ để thành tên mới đầy đủ
